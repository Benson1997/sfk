...
addSearchAlias('f', 'Kongfz', 'https://search.kongfz.com/product/?redirectFrom=product_result&actionPath=keyword,quality,sortType&quality=90~&quaSelect=2&sortType=7&page=1&keyword=', 's', 'https://duckduckgo.com/ac/?q=', function(response) {
    var res = JSON.parse(response.text);
    return res.map(function(r){
        return r.phrase;
    });
...
addSearchAlias('v', 'Bilibili', 'https://search.bilibili.com/all?keyword=', 's', 'https://duckduckgo.com/ac/?q=', function(response) {
    var res = JSON.parse(response.text);
    return res.map(function(r){
        return r.phrase;
    });
...
addSearchAlias('w', 'Wiki-zh', 'https://zh.wikipedia.org/wiki/', 's', 'https://duckduckgo.com/ac/?q=', function(response) {
    var res = JSON.parse(response.text);
    return res.map(function(r){
        return r.phrase;
    });
});
https://x.com/search?q=%E5%93%88%E5%93%88


addSearchAlias('x', 'X.com', 'https://x.com/search?src=typed_query&q=', 's', 'https://duckduckgo.com/ac/?q=', function(response) {
    var res = JSON.parse(response.text);
    return res.map(function(r){
        return r.phrase;
    });
...
// 仅在抖音取消 m 键绑定
api.unmap('m', /^https:\/\/([a-z0-9-]+\.)*douyin\.com\//i);
